...
void RobotDynamicsEstimatorDevice::publishEstimatorOutput()
{
    ...
    while (m_estimatorIsRunning)
    {
...

        {
            ...
            // m_vectorsCollectionServer.populateData("ds::measured",
            // m_estimatorInput.input.jointVelocities);
            ...
            // m_vectorsCollectionServer.populateData("tau_F::measured",
            // m_estimatorInput.input.frictionTorques);
            ...
            for (auto& [key, value] : m_estimatorOutput.output.ftWrenches)
            {
                ...
                // m_vectorsCollectionServer.populateData("fts::" + key + "::measured",
                // m_estimatorInput.input.ftWrenches[key]);
            }
            ...
            for (auto& [key, value] : m_estimatorOutput.output.linearAccelerations)
            {
                m_vectorsCollectionServer.populateData("accelerometers::" + key + "::estimated",
                                                       value);
                // m_vectorsCollectionServer.populateData("accelerometers::" + key + "::measured",
                // m_estimatorInput.input.linearAccelerations[key]);
            }

            for (auto& [key, value] : m_estimatorOutput.output.angularVelocities)
            {
                m_vectorsCollectionServer.populateData("gyroscopes::" + key + "::estimated", value);
                // m_vectorsCollectionServer.populateData("gyroscopes::" + key + "::measured",
                // m_estimatorInput.input.angularVelocities[key]);
            }
